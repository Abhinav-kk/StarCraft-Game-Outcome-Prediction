...
heading_label = tk.Label(root, text="StarCraft Winner Prediction",
...
# Race Selector Label
label_race_selector = ttk.Label(root, text="Race Selector", font=(
...
combobox_player1_race = ttk.Combobox(
    root, textvariable=player1_race_var, values=["Protoss", "Terran", "Zerg"])
combobox_player1_race.place(width=118, height=25, x=150, y=80)

# Player 2 Race Combobox
combobox_player2_race = ttk.Combobox(
    root, textvariable=player2_race_var, values=["Protoss", "Terran", "Zerg"])
combobox_player2_race.place(width=118, height=25, x=286, y=80)

# Model Selector
label_model_selector = ttk.Label(root, text="Model Selector", font=(
    "Arial", 10, "bold"), foreground="white", background="black")
...
combobox_model = ttk.Combobox(root, textvariable=model_var, values=[
                              "Random Forest", "K-Nearest Neighbors", "Support Vector Classification", "Logistic Regression", "Decision Trees", "Neural Network"])
combobox_model.place(width=160, height=25, x=550, y=80)

# Input Features Selector
label_input_features = ttk.Label(root, text="Input Features", font=(
    "Arial", 10, "bold"), foreground="white", background="black")
label_input_features.place(width=121, height=25, x=725, y=80)

# Input Features Combobox
combobox_input_features = ttk.Combobox(root, textvariable=feature_var, values=[
                                       "Simple", "Game State", "CSV"])
combobox_input_features.place(width=118, height=25, x=852, y=80)
combobox_input_features.bind(
    '<<ComboboxSelected>>', lambda event: show_frame(combobox_input_features.get()))
...
button_predict_winner = ttk.Button(
    frame1, text="Predict Winner", command=print_values1)
...
def search(event):
    ...
    if value == "":
        ...
        for item in map_names.values():
            test_str = ''.join(letter for letter in str(
                item) if letter.isalnum())
...
# Predict Winner Button
button_predict_winner = ttk.Button(
    frame2, text="Predict Winner", command=print_values2)
button_predict_winner.place(width=159, height=57, x=433, y=73)
...
instruction_label.grid(row=1, column=0, columnspan=3,
                       padx=5, pady=5, sticky="w")  # Span across all columns
...
# Predict Winner Button
button_predict_winner = ttk.Button(
    frame3, text="Predict Winner", command=print_values3, width=40)
